...
/*	hex_dump("Beacon", Msg, MsgLen);*/
    ...
    while ((Length + 2 + pEid->Len) <= MsgLen)
    {
		...
		break;
			...
			*/
            ...
            case IE_VENDOR_SPECIFIC:
                ...
                if (!memcmp(pEid->Octet, RALINK_OUI1, 3) && (pEid->Len == 7))
                {
			if (pEid->Octet[3] != 0)
		...
		/* This HT IE is before IEEE draft set HT IE value.2006-09-28 by Jan.*/
				...
				else if (!memcmp(pEid->Octet, WPS_OUI1, 4)
 				)
                {
					if (pPeerWscIe)
					{
            ...
            case IE_AP_TX_POWER:
                ...
                if (!memcmp(pEid->Octet, CISCO_OUI1, 3))
        ...
        switch(eid)
        {
	        case IE_VENDOR_SPECIFIC:
                ...
                if (ProbeReqParam->bRssiRequested &&
					 !memcmp(eid_data, RALINK_OUI1, 3) && (eid_len == 7))
                {
					if (*(eid_data + 3/* skip RALINK_OUI1 */) & 0x8)
                    	ProbeReqParam->bRssiRequested = true;
                    break;
                }
#endif /* RSSI_FEEDBACK */

                if (!memcmp(eid_data, WPS_OUI1, 4)
 					)
                {
#ifdef CONFIG_AP_SUPPORT
#endif /* CONFIG_AP_SUPPORT */
